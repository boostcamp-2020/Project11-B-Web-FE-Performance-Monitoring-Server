...
const alertMailTemplateByCount = (
  ...
  <tr>
    <td>
      <p>프로젝트에서 ${count}개의 새로운 이슈가 생겼습니다.${
    count >= 20 ? '(20개까지만 표시됩니다. 더 자세한 내용은 Admin 페이지에서 확인해주세요.)' : ''
  }</p>
      <p>이슈의 내용은 아래와 같습니다.</p>
    </td>
  </tr>${issues.slice(0, 20).reduce((html, issue) => html + issueTemplate(issue), '')}`;
...
const alertMailTemplateByPeriod = (
  ...
  <tr>
    <td>
      <p>프로젝트에서 최근 이슈가 생겼습니다.(최근 최대 10개의 이슈)</p>
      <p>이슈의 내용은 아래와 같습니다.</p>
    </td>
  </tr>
  ${issues.slice(0, 20).reduce((html, issue) => html + issueTemplate(issue), '')}`;
  return {
